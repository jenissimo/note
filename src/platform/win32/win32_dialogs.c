...
void h_dlg_find(note_host *h, int replace)
{
    ...
    h->finddlg = replace ? ReplaceTextW(&h->fr) : FindTextW(&h->fr);
}
...
int h_find_text(note_host *h, const nchar *needle, unsigned flags)
{
    ...
    found = (LONG)SendMessageW(e, EM_FINDTEXTEXW, (WPARAM)fl, (LPARAM)&ft);
    if (found < 0) {
    ...
    SendMessageW(e, EM_EXSETSEL, 0, (LPARAM)&ft.chrgText);
...
void h_goto_line(note_host *h, int line)
{
    ...
    (void)h;
    if (line < 1) line = 1;
    count = (LONG)SendMessageW(e, EM_GETLINECOUNT, 0, 0);
    if (line > count) line = count;
    idx = (LONG)SendMessageW(e, EM_LINEINDEX, (WPARAM)(line - 1), 0);
    if (idx < 0) return;
    h_sel_set(&g, idx, idx);
    SendMessageW(e, EM_SCROLLCARET, 0, 0);
    SetFocus(e);
...
void h_dlg_print(note_host *h)
{
    ...
    di.cbSize = sizeof(di);
    di.lpszDocName = (LPCWSTR)(h->app.docs[h->app.active].path[0]
                     ? note_basename(h->app.docs[h->app.active].path)
                     : (const nchar *)L"Untitled");

    if (StartDocW(pd.hDC, &di) > 0) {
        do {
            if (StartPage(pd.hDC) <= 0) break;
            next = (LONG)SendMessageW(e, EM_FORMATRANGE, TRUE, (LPARAM)&fr);
    ...
    SendMessageW(e, EM_FORMATRANGE, FALSE, 0);
    DeleteDC(pd.hDC);
...
int h_ask_save(note_host *h, const nchar *name)
{
    ...
    r = MessageBoxW(h->wnd, msg, L"note", MB_YESNOCANCEL | MB_ICONWARNING);
...
{
    MessageBoxW(h->wnd, (LPCWSTR)text, (LPCWSTR)title, MB_OK | MB_ICONINFORMATION);
}
